...
export async function generateInvoicePDF(booking: Booking, options?: InvoiceOptions | string | null): Promise<Buffer> {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const wmColor = booking.paymentStatus === "paid" ? GREEN : "#E11D48"; // green for paid, rose-600 for unpaid
      if (wmText) {
        const wmFontSize = wmText === "PAID" ? 120 : 64;
        ...
        for (let i = pages.start; i < pages.start + pages.count; i++) {
          ...
          doc.text(wmText, -250, -30, { width: 500, align: "center" });
          doc.restore();
        }
      }
