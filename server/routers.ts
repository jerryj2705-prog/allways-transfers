    ...
    create: publicProcedure
      .input(
        z.object({
          ...
          serviceType: z.enum(["airport_transfer", "sunshine_coast_airport_transfer", "hourly_hire", "point_to_point", "special_events", "freight"]),
          ...
          // Itemised components (so the stored breakdown always sums to the total)
          distanceCharge: z.number().default(0),
          outOfHoursSurcharge: z.number().default(0),
          outOfAreaSurcharge: z.number().default(0),
          fuelLevySurcharge: z.number().default(0),
          petSurcharge: z.number().default(0),
          weightSurcharge: z.number().default(0),
          roundingDiscount: z.number().default(0),
...
paymentMethod: z.enum(["stripe_prepay", "square_postpay", "cash_postpay", "direct_deposit"]),
      ...
      .mutation(async ({ input }) => {
        ...
        const booking = await createBooking({
          ...
          distanceCharge: input.distanceCharge.toFixed(2),
          outOfHoursSurcharge: input.outOfHoursSurcharge.toFixed(2),
          outOfAreaSurcharge: input.outOfAreaSurcharge.toFixed(2),
          fuelLevySurcharge: input.fuelLevySurcharge.toFixed(2),
          petSurcharge: input.petSurcharge.toFixed(2),
          weightSurcharge: input.weightSurcharge.toFixed(2),
          cardSurcharge: "0.00",
          roundingDiscount: input.roundingDiscount.toFixed(2),
